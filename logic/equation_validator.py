...
def safe_eval(expr: str):
    ...
    def _eval_node(node):
        ...
        elif isinstance(node, (ast.Num, ast.Constant)): return node.n
        elif isinstance(node, ast.UnaryOp):
            op_func = allowed_operators.get(type(node.op))
            if not op_func: raise TypeError(f"Disallowed operator: {type(node.op).__name__}")
            return op_func(_eval_node(node.operand))
        elif isinstance(node, ast.BinOp):
            op_func = allowed_operators.get(type(node.op))
            if not op_func: raise TypeError(f"Disallowed operator: {type(node.op).__name__}")
            return op_func(_eval_node(node.left), _eval_node(node.right))
        else: raise TypeError(f"Disallowed operation: {type(node).__name__}")
    ...
    def _get_canonical_form(self, equation: str) -> str:
        ...
        def _canonicalize(node):
            ...
            if isinstance(node, ast.BinOp):
                ...
                op_symbol = {ast.Add: '+', ast.Sub: '-', ast.Mult: '*', ast.Div: '/'}.get(op_type)

                if op_type in (ast.Add, ast.Mult):
                    operands = []
                    
                    def _collect_operands(sub_node):
                        if isinstance(sub_node, ast.BinOp) and type(sub_node.op) == op_type:
...
if __name__ == "__main__":
    ...
    test_cases = [
        ...
        ("10-5", "5-10", False),   # Non-commutative subtraction - should NOT be equivalent
        ("3+2*4", "2*4+3", True), # Same operands/operators, different order - should be equivalent
        ("(9+1)", "(1+9)", True), # With parentheses - should be equivalent
        ("2*3+4", "3*2+4", True), # Mixed operations - should be equivalent
        ("6/2", "2*3", False),     # Same result, different operands/operators - should NOT be equivalent
    ]
